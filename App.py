...
# 🔐 SEGURANÇA: Busca a chave nos Secrets (Nuvem) ou localmente
CHAVE_GROQ = st.secrets.get("GROQ_API_KEY", "")

# --- 2. BANCO DE DADOS (Caminho Absoluto) ---
db_path = os.path.join(os.path.dirname(__file__), 'nexus_pro_v5.db')
...
def gerar_pdf(query, analise):
    ...
    pdf.set_font("helvetica", 'B', 12); pdf.cell(0, 10, "Query Original:", ln=True)
...
# --- 4. CALLBACKS (Evita erro de StreamlitAPIException) ---
def format_callback():
...
if menu == "🧠 Oráculo":
    ...
    with st.expander("📂 CONTEXTO DE SCHEMA (Opcional)"):
        ddl = st.text_area("Cole o CREATE TABLE aqui para analise precisa:", height=100)
                ...
                with st.spinner("O Oráculo está processando..."):
                    try:
                        ...
                        res = client.chat.completions.create(
                            ...
                            messages=[
                                {"role": "system", "content": "Voce e um DBA Senior. Forneca a query otimizada em blocos ```sql."},
                                {"role": "user", "content": f"Contexto DDL:\n{ddl}\n\nQuery:\n{st.session_state.sql_input}"}
                            ]
                        ).choices[0].message.content
                        salvar(st.session_state.sql_input, res)
                        st.markdown(res)
                        botao_copiar(res)
                        st.download_button("📥 BAIXAR RELATORIO PDF", data=gerar_pdf(st.session_state.sql_input, res), file_name="analise.pdf")
                    except Exception as e: st.error(f"Erro: {e}")

elif menu == "🧪 Laboratório":
    st.title("🧪 Laboratório de Testes")
    c1, c2 = st.columns(2)
    with c1: q1 = st.text_area("Versão Original (A):", height=200, key="la")
    with c2: q2 = st.text_area("Sua Refatoração (B):", height=200, key="lb")
    if st.button("⚖️ COMPARAR PERFORMANCE"):
        if q1 and q2:
            with st.spinner("IA Comparando..."):
                client = Groq(api_key=CHAVE_GROQ)
